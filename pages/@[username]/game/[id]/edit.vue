    ...
    <section class="py-6">
      ...
      <form class="max-w-[1000px] m-auto py-6" @submit.prevent="submitGame">
          ...
          <div class="flex flex-col gap-5">
            ...
            <div class="flex flex-wrap gap-5">
              <div v-for="file in existingFiles" :key="file">
                ...
                <button @click="removeExistingFile(file)">Remove</button>
              </div>
        ...
        >
          ...
          <template v-else>Edit Game</template>
        </button>
      </form>
    </section>
  </AuthenticatedTemplate>
...
if (game.data.value?.user_id !== user.value?.id) {
  router.push("/403");
}
...
const date = ref(dayjs(game.data.value?.date).format("YYYY-MM-DD"));
const script = ref(game.data.value?.script);
const locationType = ref<"ONLINE" | "IN_PERSON">(
  game.data.value?.location_type || "ONLINE"
);
const location = ref(game.data.value?.location);
const community = ref(game.data.value?.community);
const playerCount = ref(game.data.value?.player_count);
...
>(
  game.data.value?.player_characters.map((character) => ({
    id: character.id,
    name: character.name,
...
const final3 = ref(game.data.value?.final3);
const win = ref(game.data.value?.win);
const notes = ref(game.data.value?.notes);
const files = ref<File[]>([]);
const existingFiles = ref<string[]>(game.data.value?.image_urls || []);
...
function removeExistingFile(file: string) {
...
async function submitGame() {
  ...
  const urls = [...existingFiles.value];
  ...
  const toRemove = game.data.value?.image_urls.filter(
  ...
  const result: Response = await fetch(`/api/games/${game.data.value?.id}`, {
    method: "PUT",
    body: JSON.stringify({
