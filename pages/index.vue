<template>
  <template v-if="games.length">
    <table class="w-11/12 m-auto my-6">
      ...
      <tbody>
        <tr v-for="game in stats" :class="rowHighlight(game)">
          ...
          <td>{{ game.final3 === null ? "-" : game.final3 ? "Yes" : "No" }}</td>
          <td>{{ game.win ? "Yes" : "No" }}</td>
        </tr>
      </tbody>
    </table>
    <div class="flex flex-wrap items-center space-around gap-4 justify-center">
      <Alignment class="w-1/4" :games="games" />
      <WinRate class="w-1/4" :games="games" />
    </div>
  </template>
  <template v-else>
