import { PrismaClient, Alignment, WinStatus_V2, Role } from "@prisma/client";
...
export async function roleStats(
  ...
  const with_ids = options?.withIds;
  ...
  const games = (
    ...
    if (with_ids && with_ids.length > 0) {
      return with_ids.every((role_id) =>
        game.grimoire.some((grimoire) =>
          grimoire.tokens.some((token) => token.role_id === role_id)
        )
      );
    } else {
      return true;
  ...
  for (const script of popular_scripts) {
    ...
    const pct = total > 0 ? +((wins / total) * 100).toFixed(2) : 0;
  ...
  return {
    ...
    win_loss: {
      ...win_loss,
      pct:
        win_loss.wins + win_loss.losses > 0
          ? +(
              (win_loss.wins / (win_loss.wins + win_loss.losses)) *
              100
            ).toFixed(2)
          : 0,
    },
