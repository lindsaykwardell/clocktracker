<template>
  <div class="p-4 border rounded-lg dark:border-stone-700/50 bg-stone-300/30 dark:bg-stone-900/40">
    ...
    <div class="h-36 md:h-48">
      <Pie
        id="storyteller-script-size-minions"
        :data="chartData"
...
function getGameSizeKey(
  ...
  if (playerCount <= 6) {
    ...
    return "two"; // 2 minions
  } else {
    return "three"; // 3 minions
  }
}
...
/**
 * Build gameSizes based on player_count, with splits inside per (player_count, traveler_count).
 */
const gameSizes = computed<Bucket[]>(() => {
  // GameSizeKey -> (splitKey -> SplitInfo)
  ...
  for (const [key, splitMap] of gameSizeMap.entries()) {
    const splits = Array.from(splitMap.values()).sort((a, b) => {
      // Sort by playerCount, then travelerCount
  ...
  // Keep gameSizes in a logical order.
        ...
        label(context: any) {
          ...
          for (const split of gameSize.splits) {
            ...
            lines.push(
              `${count} game${
                count === 1 ? "" : "s"
              } with ${playerCount}${travelersPart} players`
            );
          }
