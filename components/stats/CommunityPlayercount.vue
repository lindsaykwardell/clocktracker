...
const groups = computed<PlayerCountGroup[]>(() => {
  ...
  for (const game of props.games) {
    ...
    if (!splitMap.has(travelerCount)) {
      splitMap.set(travelerCount, { playerCount, travelerCount, count: 0 });
    }
    splitMap.get(travelerCount)!.count += 1;
  }
  ...
  for (const [effectiveCount, splitMap] of groupMap.entries()) {
    ...
    const label = effectiveCount === 1 ? "1 player" : `${effectiveCount} players`;
